...
def process_elevations(element):
    ...
    for elevation_element in elevation_elements:
        ...
        X = e[index]
        S = [a, b, c, d]
        restored_Q = [x for _, x in sorted(zip(X, S))]

        elevation_element.set("a", restored_Q[0])
        elevation_element.set("b", restored_Q[1])
        elevation_element.set("c", restored_Q[2])
        elevation_element.set("d", restored_Q[3])
...
if __name__ == "__main__":
    # 调用函数
    xodr_file = r"E:\SanFrancisco-Downtown(333).xodr"
    output_file = r"E:\SanFrancisco-Downtown(8).xodr"
