...
def process_widths(element):
    ...
    for lane_element in lane_elements:
        ...
        for width_element in width_elements:
            ...
            width_element.set("a", restored_Q[0])
            width_element.set("b", restored_Q[1])
            width_element.set("c", restored_Q[2])
            width_element.set("d", restored_Q[3])


if __name__ == "__main__":
    # 调用函数
    xodr_file = r"E:\SanFrancisco-Downtown(33).xodr"
    output_file = r"E:\SanFrancisco-Downtown(333).xodr"
